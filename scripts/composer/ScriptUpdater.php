...
class ScriptUpdater
{
  ...
  public static function createParentFiles(Event $event)
  {
    ...
    if ($fs->exists($pantheon_install)) {
      $iterator = $finder->files()->in($pantheon_install);

      foreach ($ignored_files['km-collab-scaffold']['excludes'] as $ignore_file) {
        $iterator->notName($pantheon_install . '/' . $ignore_file);
      }

      foreach ($iterator as $file) {
        if (!in_array($file->getBasename(), $ignored_files['km-collab-scaffold']['excludes'])) {
